...
describe("services > dnpDirectory, whitelisted mechanism", () => {
  ...
  it("Should add a dnpDirectory, whitelist it and retrieve it", () => {
    const dnps = [{ name: "dnpA" }, { name: "dnpB" }];
    const expectedState = {
      dnpA: { name: "dnpA", whitelisted: true },
      dnpB: { name: "dnpB", whitelisted: true }
    };
    ...
    expect(s.getDnpDirectoryWhitelisted({ [mountPoint]: state })).toEqual(
      expectedState
    );
  });
});
